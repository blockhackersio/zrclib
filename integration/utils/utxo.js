...
class Utxo {
   ...
   */
  constructor({ amount = 0, keypair = null, blinding = randomBN(), index = null } = {}) {
  ...
  async getCommitment() {
    if (!this._commitment) {
      this._commitment = await poseidonHash([this.amount, this.keypair.pubkey, this.blinding])
    }
  ...
  async getNullifier() {
    if (!this._nullifier) {
      ...
      const signature = this.keypair.privkey ? await this.keypair.sign(commitment, this.index || 0) : 0
      this._nullifier = await poseidonHash([await this.getCommitment(), this.index || 0, signature])
    }
